...
def get_ontology_field_name(ontology_id:str, term_id:str, collection:str):
    ...
    results = client[beacon_db].get_collection(collection).find(query).limit(1)
...
def insert_all_ontology_terms_used():
    collections = client[beacon_db].list_collection_names()
    ...
    for c_name in collections:
        if c_name not in ['counts', 'similarities', 'synonyms']:
            ...
            if len(terms) > 0:
                client[beacon_db].filtering_terms.insert_many(terms)

def find_ontology_terms_used(collection_name: str) -> List[Dict]:
    print(collection_name)
    terms_ids = []
    count = client[beacon_db].get_collection(collection_name).estimated_document_count()
    if count < 10000:
        num_total=count
    else:
        num_total=10000
    ...
    if count > 10000:
        while i < count:
            xs = client[beacon_db].get_collection(collection_name).find().skip(i).limit(10000)
        ...
        xs = client[beacon_db].get_collection(collection_name).find().skip(0).limit(10000)
        for r in tqdm(xs, total=num_total):
            matches = ONTOLOGY_REGEX.findall(str(r))
            icd_matches = ICD_REGEX.findall(str(r))
            for ontology_id, term_id in matches:
...
def merge_ontology_terms():
    filtering_terms = client[beacon_db].filtering_terms.find({"type": "ontology"})
    array_of_ids=[]
    repeated_ids=[]
    new_terms=[]
    ...
    for repeated_id in repeated_ids:
        repeated_terms = client[beacon_db].filtering_terms.find({"id": repeated_id, "type": "ontology"})
        array_of_scopes=[]
        ...
        client[beacon_db].filtering_terms.delete_many({"id": repeated_id})
    if new_terms != []:
        client[beacon_db].filtering_terms.insert_many(new_terms)
        
    
def merge_alphanumeric_terms():
    filtering_terms = client[beacon_db].filtering_terms.find({"type": "alphanumeric"})
    array_of_ids=[]
    repeated_ids=[]
    new_terms=[]
    for filtering_term in filtering_terms:
    ...
    #print("repeated_ids are {}".format(repeated_ids))
    for repeated_id in repeated_ids:
        repeated_terms = client[beacon_db].filtering_terms.find({"id": repeated_id, "type": "alphanumeric"})
        array_of_scopes=[]
        for repeated_term in repeated_terms:
            #print(repeated_term)
            id=repeated_term["id"]
...
def merge_custom_terms():
    filtering_terms = client[beacon_db].filtering_terms.find({"type": "custom"})
    array_of_ids=[]
    repeated_ids=[]
    new_terms=[]
    for filtering_term in filtering_terms:
    ...
    #print("repeated_ids are {}".format(repeated_ids))
    for repeated_id in repeated_ids:
        repeated_terms = client[beacon_db].filtering_terms.find({"id": repeated_id, "type": "custom"})
        array_of_scopes=[]
        for repeated_term in repeated_terms:
            #print(repeated_term)
            id=repeated_term["id"]
