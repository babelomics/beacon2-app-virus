...
def cross_query(query: dict, scope: str, collection: str, request_parameters: dict):
    if scope == 'genomicVariation' and collection == 'g_variants' or scope == collection[0:-1]:
        ...
        if request_parameters != {}:
            biosample_ids = client[beacon_db].genomicVariations.find(request_parameters, {"caseLevelData.biosampleId": 1, "_id": 0})
            final_id='id'
            ...
            LOG.debug(subquery)
            mongo_collection=client[beacon_db].biosamples
        ...
        if scope == 'individual' and collection == 'g_variants':
            mongo_collection=client[beacon_db].individuals
            original_id="id"
            ...
            biosample_ids = client[beacon_db].genomicVariations.find(query, {"caseLevelData.biosampleId": 1, "_id": 0})
            final_id='id'
            original_id="biosampleId"
            def_list=[]
            for iditem in biosample_ids:
            ...
            mongo_collection=client[beacon_db].runs
        ...
        try:
            if len(request_parameters["$or"]) >= 1:
                ...
                for reqpam in request_parameters["$or"]:
                    biosample_ids = client[beacon_db].genomicVariations.find(reqpam, {"caseLevelData.biosampleId": 1, "_id": 0})
...
def apply_ontology_filter(query: dict, filter: OntologyFilter, collection: str, request_parameters: dict) -> dict:
    ...
    if filter.similarity != Similarity.EXACT:
        ...
        try:
            if filter.similarity == Similarity.HIGH:
                similarity_high=[]
                ontology_dict=client[beacon_db].similarities.find({"id": filter.id})
    ...
    if filter.include_descendant_terms == True:
        ...
        list_descendant = []
        try:
            ontology_dict=client[beacon_db].similarities.find({"id": ontology})
