...
def get_count(collection: Collection, query: dict) -> int:
    if not query:
        ...
        counts=client[beacon_db].counts.find({"id": str(query), "collection": str(collection)})
        try:
            counts=list(counts)
            if counts == []:
                match_dict={}
                ...
                insert_total=client[beacon_db].counts.insert_one(insert_dict)
            else:
                total_counts=counts[0]["num_results"]
        except Exception:
            try:
                total_counts=client[beacon_db].counts.count_documents(query)
                insert_dict={}
                insert_dict['id']=str(query)
                insert_dict['num_results']=total_counts
                insert_dict['collection']=str(collection)
