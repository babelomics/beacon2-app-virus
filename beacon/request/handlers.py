...
def collection_handler(db_fn, request=None):
    async def wrapper(request: Request):
        try:
            ...
            LOG.debug(count)
            LOG.debug(records)
            LOG.debug(qparams.query.test_mode)
    ...
    async def wrapper(request: Request):
        try:
            ...
            if qparams.query.test_mode:
                ...
                _, _, authorized_datasets_tmp = get_datasets(None, qparamsAll)
                authenticated = True
                username = ''
                authorized_datasets = [ r['id'] for r in authorized_datasets_tmp ]
                """
                ...
                with ThreadPoolExecutor() as pool:
                    LOG.debug([loop.run_in_executor(pool, db_fn, entry_id, qparams, dataset) for dataset in response_datasets])
